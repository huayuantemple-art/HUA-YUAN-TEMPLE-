...
最新公告頁 SHALL 以月曆為唯一內容檢視(原列表檢視移除,跑馬燈與分類篩選保留)。月曆 SHALL 以月格呈現、可切換上一月/下一月,僅於月格標記 `date` 可解析為合法日期(格式 `YYYY.MM.DD`)之已發布公告(後台輸入公告日期 MUST 維持 `YYYY.MM.DD` 格式)。月曆下方 SHALL 依當前分類列出公告:未點選日期時依日期倒序分組列出該分類全部公告,點選日期時僅列該日並提供「顯示全部」返回。月曆之視覺 MUST 嚴格遵循既有 UI style(色彩、字體、版面),且 MUST NOT 引入外部日曆套件或後端變更。

#### Scenario: 進入公告頁即顯示月曆與完整列表
- **WHEN** 使用者進入最新公告頁
- **THEN** 顯示月格月曆,有已發布公告之日期帶有標記;月曆下方依日期倒序列出全部已發布公告
...
#### Scenario: 點擊日期縮小至當日
- **WHEN** 使用者點擊月曆上帶標記之日期
- **THEN** 月曆下方僅顯示該日公告(標題、標籤、內容,依建立時間倒序);再點一次該日期或按「顯示全部」回到該分類完整列表
