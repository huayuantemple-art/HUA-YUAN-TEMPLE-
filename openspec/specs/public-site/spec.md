...
最新公告頁 SHALL 以月曆為唯一內容檢視(原列表檢視移除,跑馬燈與分類篩選保留)。月曆 SHALL 以月格呈現、可切換上一月/下一月,並於月格日期下方以菱形標記該月有公告的日子;僅標記 `date` 可解析為合法日期(格式 `YYYY.MM.DD`)之已發布公告(後台輸入公告日期 MUST 維持 `YYYY.MM.DD` 格式)。月曆下方列表 SHALL 與顯示中月份對應:未點選日期時依日序列出該月該分類全部公告,點選日期時僅列該日並提供「顯示全部」返回;切換月份時列表隨之更換並清除日期選取。初始月份 SHALL 錨定今日之後最近有公告的月份(無未來公告則為最近的過去公告月份)。跑馬燈內容 SHALL 與月曆分離:可由後台網站文案(`news_marquee`,每行一則)獨立管理,留空時 fallback 為自動輪播最新公告標題前 6 筆。月曆之視覺 MUST 嚴格遵循既有 UI style(色彩、字體、版面),且 MUST NOT 引入外部日曆套件或後端變更。

#### Scenario: 跑馬燈內容獨立於月曆
- **WHEN** 管理者於後台網站文案設定「跑馬燈內容」(每行一則)
- **THEN** 前台跑馬燈輪播該內容,月曆與下方列表不受影響;清空該文案則恢復輪播最新公告標題

#### Scenario: 進入公告頁即顯示月曆與當月列表
- **WHEN** 使用者進入最新公告頁
- **THEN** 月曆落在今日之後最近有公告的月份,有公告之日期帶菱形標記;月曆下方依日序列出該月公告

#### Scenario: 分類篩選作用於月曆與下方列表
- **WHEN** 使用者選擇分類(課程/法會/公告)
- **THEN** 月格標記與月曆下方列表僅呈現該分類公告;選「全部」則涵蓋所有分類

#### Scenario: 切換月份列表隨動
- **WHEN** 使用者以 ‹ › 切換月份
- **THEN** 月格標記與下方列表更換為該月內容;該月無公告時顯示「本月無公告」提示

#### Scenario: 點擊日期縮小至當日
- **WHEN** 使用者點擊月曆上帶標記之日期
- **THEN** 月曆下方僅顯示該日公告(標題、標籤、內容,依建立時間倒序);再點一次該日期或按「顯示全部」回到該月列表

#### Scenario: 無效日期公告不進月格
- **WHEN** 公告之 `date` 無法解析為合法日期(格式錯誤或日期不存在)
- **THEN** 該公告不出現於月格標記,於下方列表以「日期未定」分組列於最末(跑馬燈仍照常輪播其標題)
